...
struct VirtualRegion::Init {
      ...
      uint64_t alignment, uint64_t init_chunks)
  {
    ...
    if (chunk_shift == 0) {
      chunk_shift = (uint64_t)int32Log2(page_size);
    }
    ...
    uint64_t max_chunks = divideRoundUp(max_bytes, chunk_size);
    ...
    STATIC_UNIMPLEMENTED();
#endif
    uintptr_t aligned_base = roundUp((uintptr_t)base,
        (uintptr_t)alignment);
...
static uint64_t computeChunkShift(uint32_t bytes_per_item)
{
  ...
  uint64_t target_shift = int64Log2(target_total_bytes);
...
VirtualStore::VirtualStore(uint32_t bytes_per_item,
  ...
  : region_((uint64_t)bytes_per_item * (uint64_t)max_items,
    ...
    data_((void *)roundUp((uintptr_t)region_.ptr() + start_offset,
